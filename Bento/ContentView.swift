...
func rects(item: SplitItem, proposal: ProposedViewSize, subviews: Layout.Subviews.SubSequence, axis: Axis, origin: CGPoint) -> [CGRect] {
    if item.children.isEmpty {
        return [.init(origin: origin, size: subviews[0].sizeThatFits(proposal))]
    }
    var result: [CGRect] = []
    let dividedProposal = proposal / .init(item.children.count)
    ...
    for child in item.children {
        let endIndex = remainingSubviews.startIndex+child.numberOfChildren
        let childSubviews = remainingSubviews[remainingSubviews.startIndex..<endIndex]
        let childRects = rects(item: child, proposal: childProposal, subviews: childSubviews, axis: axis.other, origin: currentPosition)
        result.append(contentsOf: childRects)
        remainingSubviews.removeFirst(child.numberOfChildren)
        if axis == .vertical {
            let actualHeight = childRects.map { $0.height }.max() ?? .zero
            currentPosition.y += actualHeight
        } else {
            let actualWidth = childRects.map { $0.width }.max() ?? .zero
            currentPosition.x += actualWidth
        }
    }
    return result
}
...
struct BentoLayout: Layout {
    ...
    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) -> CGSize {
        let allRects = rects(item: split, proposal: proposal, subviews: subviews[...], axis: .vertical, origin: .zero)
        return allRects.reduce(CGRect.null, { $0.union($1) }).size
    }

    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) {
        let allRects = rects(item: split, proposal: proposal, subviews: subviews[...], axis: .vertical, origin: bounds.origin)
